...
def native_extension(name):
    candidates = sorted(glob.glob(f'build/lib.win*/{name}*'))
    if not candidates:
        raise FileNotFoundError(
            f"Native extension not found for {name}. Run python3.exe setup-win.py build first.")
    return candidates[-1]
